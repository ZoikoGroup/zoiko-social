...
const CARDS = [
  {
    ...
    description: "Check the verified organization status and view its profile before you start an inquiry.",
    warn: false,
    icon: (
  ...
  {
    id: "accountable-comm",
    title: "Keep communication accountable",
    description: "Use auditable platform channels where available until you're ready for appropriate next steps.",
    warn: false,
    icon: (
  ...
  },
  {
    ...
    description: "Be cautious of rushed transfers, unexpected payment demands, or off- platform pressure that conflicts with this guidance.",
    warn: false,
    icon: (
      <>
  ...
  },
  {
    ...
    description: "Don't share more personal information than necessary. Private addresses and sensitive details stay protected.",
    warn: false,
    icon: (
      <>
  ...
  },
  {
    ...
    description: "Each rescue or shelter may use its own application, home-check, reference, fee, or meeting process where lawful.",
    warn: false,
    icon: (
  ...
  },
  {
    ...
    description: "Report a Concern is available from listings, organization profiles, and this safety content.",
  ...
  {
    id: "emergency-issue",
    title: "Emergency welfare issue",
    description: "If there is immediate danger to an animal or person, contact your local emergency or animal-welfare authority directly.",
    warn: true,
    icon: (
...
export default function AdoptionSafety() {
  return (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {CARDS.map((card) => (
          ...
          >
            ...
            {card.link && (
              <div className="text-cyan-800 text-xs font-bold font-['Plus_Jakarta_Sans'] cursor-pointer mt-auto">
                {card.link} &gt;
              </div>
            )}
          </div>
