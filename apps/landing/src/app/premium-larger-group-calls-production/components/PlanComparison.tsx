const PLANS = [
  {
    ...
    features: [
      { text: "Larger Group Calls", included: false },
      { text: "Up to 8 participants", included: true },
  ...
  },
  {
    ...
    priceNote: "per month",
    features: [
      { text: "✓ Larger Group Calls (50+)", included: true },
      { text: "✓ Advanced host controls", included: true },
  ...
  },
  {
    ...
    features: [
      ...
      { text: "✓ Enhanced call capacity", included: true },
...
/**
 * "Choose your Premium plan" — three pricing cards. Figma: desktop
 * 732:4625, mobile 732:4980. Identical copy across both breakpoints
 * (confirmed plan names, prices, and feature lines match); only layout
 * changes — a 3-column row on desktop vs. a stacked, full-width column on
 * mobile. The middle "Premium" card carries a #066879 2px highlight border
 * on both breakpoints. Section background white, horizontal padding 105px
 * desktop / 24px mobile.
 */
export default function PlanComparison() {
  return (
    <section className="w-full bg-[#f7f9fa] py-12 lg:bg-white lg:py-20">
      <div className="mx-auto w-full max-w-[1440px] px-6 lg:px-[105px]">
        ...
        <div className="mt-8 flex flex-col gap-6 lg:mt-[46px] lg:flex-row lg:justify-center lg:gap-6">
          {PLANS.map((plan) => (
            <div
              key={plan.name}
              className={`flex w-full flex-col overflow-hidden rounded-[20px] border bg-white lg:w-[372px] ${
            ...
            >
              <div className="border-b border-[#dce5e8] bg-[#f7f9fa] px-6 pb-6 pt-[23px]">
                ...
                <p className="font-jakarta text-[12px] font-normal leading-[19.2px] text-[#5e7076]">
                  {plan.priceNote}
                </p>
              </div>
        ...
        <p className="mt-6 text-center font-jakarta text-[12px] font-normal leading-[19.2px] text-[#5e7076] lg:mt-8">
